...
namespace funcs{
	...
	*/
	...
	GLuint genWorleyNoise(unsigned int width, unsigned int height, unsigned int breadth, unsigned int num_tiles){
		...
		std::ifstream file("3d_texture.raw", std::ios::binary);
		if (!file){
		...
		std::cout << info.DEPTH << ' ' <<  info.HEIGHT<< ' ' << info.WIDTH  << std::endl;
				//   << ' ' << info.CHANNELS << std::endl;

		size_t sz = info.DEPTH * info.HEIGHT * info.WIDTH * sizeof(uint8_t);
		...
		glTexImage3D(GL_TEXTURE_3D, 0, GL_RED, info.WIDTH, info.HEIGHT, info.DEPTH, 0, GL_RED, GL_UNSIGNED_BYTE, data.data());
