...
def worleyNoiseFbm(x, y, z):
    ...
    pnoise = noise.pnoise3(x * 10, y * 10, z * 10, octaves = 4, lacunarity = 2., repeatx = 10, repeaty = 10, repeatz = 10, base=0)
    wfbm = 1-(worley0 * .625 + worley1 * .25 + worley2 * .125)

    perlinworley = remap(pnoise, .0, 1., wfbm, 1.)

    WFBM0 = 1-((worley1 + worley2 * .5 + worley3 * .25)/(1 + .5 + .5*.5))
    WFBM1 = 1-((worley2 + worley3 * .5 + worley4 * .25)/(1 + .5 + .5*.5))
    WFBM2 = 1-((worley3 + worley4 * .5 + worley5 * .25)/(1 + .5 + .5*.5))
