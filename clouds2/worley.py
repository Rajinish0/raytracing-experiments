...
if len(sys.argv) < 4:
	print(f"USAGE {sys.argv[0]} output-file-name texture-file-name quality [NUM_TILES=10]")
	sys.exit()
			...
			for y in range(H):
				for x in range(W):
					...
					for i in range(-1, 2):
						for j in range(-1, 2):
							for k in range(-1, 2):
								neighborX = (tileX + j) % NUM_TILES
								neighborY = (tileY + i) % NUM_TILES
								neighborZ = (tileZ + k) % NUM_TILES
								anchor_x, anchor_y, anchor_z = random_f((neighborY, neighborX,neighborZ ) )
								anchor_x = (tileX + j + anchor_x) * BOX_WIDTH
								anchor_y = (tileY + i + anchor_y) * BOX_HEIGHT
								anchor_z = (tileZ + k + anchor_z) * BOX_DEPTH
								min_d = min(min_d, distSqr( (x, y, z), (anchor_x, anchor_y, anchor_z) ) )
...
def generateWorley3d_lowres():
	D,H,W = (32,)*3
	arr = np.empty((D, H, W, 3), np.uint8)
	TILES = [4, 8, 16]
	for a in range(3):
		NUM_TILES = TILES[a]
		BOX_WIDTH  =  W / NUM_TILES
		...
		for z in range(D):
			for y in range(H):
				for x in range(W):
					...
					color += (min_d/MAX_DIST)
					arr[z, y, x, a] = int((1 - color) * 255)
					print("{:.2f}".format((a*W*D*H + z*W*H + y * W + x)/(H*W*D*3)), end='\r')
	with open(TEXTURE_FILE, 'wb') as f:
		f.write(np.array([D, H, W, 3], dtype=np.int32).tobytes())
		f.write(arr.tobytes())
	for j in range(3):
		for i in range(5):
			Image.fromarray(arr[i, :, :, j]).save(OUTPUT_NAME.split('.')[0] + "_LOW_" + str(j) + "_" + str(i) + '.png' )
		print("Saved the picture")
...
if __name__ == '__main__':
	if QUALITY.lower() == 'high':
		...
		generateWorley3d_lowres()
	else:
		print("Invalid quality val, should be high/low")
